...
const FloatingDockDesktop = ({ items, className }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-16 gap-4 items-end rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 px-4 pb-3",
        className
      )}
    >
      {items.map((item) => (
  ...
  return (
    <Link to={href} target="_blank">
      <motion.div
        ...
        className="aspect-square rounded-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center relative"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
